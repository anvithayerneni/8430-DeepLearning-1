...
def data_p():
    filepath = '/home/ayernen/assignment2/MLDS_hw2_1_data/'
...
def annot(label_file, wd, w2i):
    lj = '/home/ayernen/assignment2/MLDS_hw2_1_data/' + label_file
    ac = []
    with open(lj, 'r') as f:
        label = json.load(f)
    for d in label:
...
def test(test_loader, model, i2w):
    ...
    for batch_idx, batch in enumerate(test_loader):
        ...
        test_predictions = seq_predictions.cpu().numpy()
...
def main():
    ...
    train_dataloader = DataLoader(dataset = train_dataset, batch_size=64, shuffle=True, num_workers=8, collate_fn=split_batch)
    
    epochs_n = 50

    encoder = encoderRNN()
    decoder = decoderRNN(512, len(i2w) +4, len(i2w) +3, 1024, 0.3)
